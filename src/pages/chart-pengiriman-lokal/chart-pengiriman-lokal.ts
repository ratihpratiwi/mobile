...
@Component({
  selector: 'page-chart-pengiriman-lokal',
  templateUrl: 'chart-pengiriman-lokal.html',
})
export class ChartPengirimanLokalPage {
  @ViewChild('barCanvas') barCanvas;
  public viewState: string = "chart";
  public pengiriman: any;
  public pengirimanAvg: any;
  dataPengiriman: any;
  ...
  initialize(): void {
    ...
    this.dashboardServiceProvider.getDataPengiriman().toPromise().then(res => {
      var that = this;
      this.pengiriman = _.chain(res.pengiriman)
        .groupBy(function (p) {
          return p;
      ...
      this.pengirimanAvg = _.chain(res.pengiriman)
        .groupBy(function (p) {
          return p;
        }).map(function (value, key) {
          return {
          ...
          return {
            "company": p.company, "labels": day, "data": dtsPrice
          }
        }).value();
    });
  }

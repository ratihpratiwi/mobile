...
@Component({
  selector: 'page-chart-hpp-harian',
  templateUrl: 'chart-hpp-harian.html',
})
export class ChartHppHarianPage {
      ...
      this.hpp = _.chain(res.hpp)
        .groupBy(function (p) {
          ...
          var dates = _.map(p.data, function (date) {
            return moment(moment(date.period, "YYYY-MM-DD").format("YYYY-MM-DD"));
          });
          var index = [];
